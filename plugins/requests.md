...
<p>add_action( 'rest_api_init', 'prefix_register_book_route' ); </p>
<p>/**
...
<p>// A simple function that grabs a book title from our books by ID.
...
<p>add_action( 'rest_api_init', 'prefix_register_batch_route' );</p>
<p>/**
 * Our registered endpoint callback. Notice how we are passing in $request as an argument.
 * By default, the WP_REST_Server will pass in the matched request object to our callback.
 *
...
function prefix_handle_batch_requests( $requests ) {
	$data = array();</p>
<p>	// Foreach request specified in the requests param run the endpoint.
...
<p>	return rest_ensure_response( $data );
}</p>
<p>/**
 * This handles the building of the response for the batch requests we make.
 *
...
function prefix_handle_request( $request_params ) {
	$request = new WP_REST_Request( $request_params['method'], $request_params['route'] );</p>
<p>	// Add specified request parameters into the request.
...
<p>function prefix_validate_requests( $requests, $request, $param_key ) {
...
<p>		if ( isset( $request['params'] ) &amp;&amp; ! is_array( $request['params'] ) ) {
...
<p>	// This is a black listing approach to data validation.
	return true;
}
```
